...
from PyQt6.QtWidgets import QDialog, QApplication, QMainWindow, QFileDialog, QProgressBar, QSplashScreen
import sqlite3
from PyQt6.QtWidgets import QMessageBox, QWidget, QTableWidgetItem, QVBoxLayout, QHBoxLayout, QPushButton, QCheckBox, QHeaderView
from PyQt6.QtGui import QHideEvent, QIcon, QMovie, QPaintEvent, QPainter, QPixmap, QShowEvent, QColor
...
class UnitTableView(QDialog):
    ...
    def DeleteSelectOrders(self):
        if self.tableWidget.selectionModel().hasSelection():
            reply = QMessageBox.question(self, 'ВНИМАНИЕ !!!',"Вы точно хотите удалить запись ?")
            choice = reply.value    
            if choice == 16384:
                row = self.tableWidget.currentRow()
                id = self.tableWidget.item(row,0).text()
...
class CreateDataBaseWindow(QWidget):
    ...
    def CreateDataBase(self):
        ...
        shutil.copyfile('database.db', name)
...
class MainTableView(QDialog):
    ...
    def CheckDataBaseDefault(self):
        ...
        if path.name != 'database.db' and path.name != "":
