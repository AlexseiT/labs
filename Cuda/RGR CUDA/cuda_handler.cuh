...
void transpose_check(float* input_matrix, float* transposed_matrix)
{
    ...
    if (is_equal)
        cout << GREEN << "SUCCESS: Transposed matrix equal to expected" << STANDART_COLOR <<
        endl;
    else
        cout << RED << "FAILURE: Transposed matrix doesn't equal to expected" << STANDART_COLOR <<
        endl;
}
...
float thrust_transpose(float* input_matrix, float* output_matrix)
{
    ...
    auto input_iter = thrust::make_permutation_iterator(input.begin(),
        thrust::make_transform_iterator(first,
            [=] __host__ __device__(int i) mutable
    {
        int r = i / MATRIX_SIZE;
        int c = i % MATRIX_SIZE;
...
float cublas_transpose(float* input_matrix, float* output_matrix)
{
    ...
    cudaEventRecord(start);
    cublasSgeam(handle, CUBLAS_OP_T, CUBLAS_OP_N, MATRIX_SIZE, MATRIX_SIZE, &alpha,
        input_matrix, MATRIX_SIZE, &beta, input_matrix, MATRIX_SIZE, output_matrix, MATRIX_SIZE);
    cudaEventRecord(stop);
...
void multiply_check(float* input_matrix_1, float* input_matrix_2, float* result_matrix)
{
    ...
    if (is_equal)
        ...
        cout << RED << "FAILURE: Result matrix doesn't equal to expected" << STANDART_COLOR <<
        endl;
}
...
float thrust_matrix_multiply(float* input_matrix_1, float* input_matrix_2, float* output_matrix)
{
        ...
        [d_input_matrix_1, d_input_matrix_2] __device__(int idx) -> float
    {
        ...
        for (int i = 0; i < MATRIX_SIZE; i++)
        {
            sum += d_input_matrix_1[row * MATRIX_SIZE + i] * d_input_matrix_2[i * MATRIX_SIZE +
                col];
        }
        return sum;
    });
...
float cublas_matrix_multiply(float* input_matrix_1, float* input_matrix_2, float* output_matrix)
{
    ...
    cublasSgemm(handle, CUBLAS_OP_N, CUBLAS_OP_N, MATRIX_SIZE, MATRIX_SIZE,
        MATRIX_SIZE, &alpha, input_matrix_2, MATRIX_SIZE, input_matrix_1, MATRIX_SIZE, &beta,
        output_matrix, MATRIX_SIZE);
...
/*Scalar multiplication of matrices is the operation of multiplying each element of a matrix by a scalar value (a
single number).
In other words, given a matrix A and a scalar k, the scalar multiplication of A and k is a new matrix C
where each element of A is multiplied by k to get the corresponding element of C. For example, if A = [1 2; 3
4] and k = 2,
...
float cublas_scalar_multiply(float* input_matrix, float* output_matrix)
{
    ...
    cudaMemcpy(output_matrix, input_matrix, MATRIX_SIZE * MATRIX_SIZE * sizeof(float),
