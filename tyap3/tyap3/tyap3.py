...
class TableView(QWidget):
    def __init__(self, countStates):
        ...
        for row in range(countStates):
            for column in range(6):
                ...
                if column < 3:
                    self.table.item(row, column).setBackground(QColor(50,100,50))
                if column >= 3:
                    self.table.item(row, column).setBackground(QColor(100,50,100))
    ...
    def RunMachine(self,chainElement,textAdd):
        ...
        self.textEnd.setText(string + " [ Цепочка пренадлежит заданому языку - конечное состояние, стек пустой, входная цепочка пуста ]")
        return True

    def Load(self):
        fileDMP = open("san.txt", "r")
    ...
    def Enter(self):
        ...
        if (separator == ""):
            separator = " "
        chainArr = self.chain.text().split(separator)
